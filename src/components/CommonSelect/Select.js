...
export default function CommonSelect(props) {
    ...
    return <Select {...otherProps}>
        {
            isShowFirstCustomOption && <Option key={firstOptionValue} value={firstOptionValue}>{firstOptionName}</Option>
        }
        {
            dataSource.map(item => <Option key={item.id} value={item.id}>{item.name}</Option>)
        }
        {
            isShowEndCustomOption && <Option key={endOptionValue} value={endOptionValue}>{endOptionName}</Option>
        }
    </Select>
}
